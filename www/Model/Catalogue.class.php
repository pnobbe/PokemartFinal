...
class Catalogue
{
    ...
    public function getSearchResults($criteria)
    {
        ...
        $rows = array();
        $val = "%" . $criteria . "%";
        $res = Database::query_safe("SELECT * FROM `items` WHERE (`Name` LIKE ? OR `DescriptionLong` like ? OR `DescriptionShort` like ? or `Price` like ?) AND `Active` = TRUE", array($val, $val, $val, $val));
        foreach ($res as $val) {
